...
/**
 * Handles all the click listeners, as well as delegates all map related functions to the
 * {@link com.example.towing.towme.maps.Placer} class
 */
...
public class MapFragment extends Fragment implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        TowLocationClient.setActivity(getActivity());
    }
    ...
    public void onStart() {
        super.onStart();
        if (!TowLocationClient.hasLocationListener(this))
            TowLocationClient.addLocationListener(this);
    }
...
//        TowLocationClient.resume();
     ...
     * Sets up the map if it is possible to do so (i.e., the Google Play services APK is correctly
     * installed) and the map has not already been instantiated.. This will ensure that we only ever
     * call onLocationChanged once when {@link #mMap} is not null.
     * <p/>
     * If it isn't installed {@link com.google.android.gms.maps.SupportMapFragment} (and
     * {@link com.google.android.gms.maps.MapView MapView}) will show a prompt for the user to
     * install/update the Google Play services APK on their device.
